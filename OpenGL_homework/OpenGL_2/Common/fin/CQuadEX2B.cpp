...
void CQuadEX2B::SetShader(mat4 &mxModelView, mat4 &mxProjection, GLuint uiShaderHandle)
{
    // Load shaders and use the resulting shader program
	if( uiShaderHandle == MAX_UNSIGNED_INT) m_uiProgram = InitShader("vsVtxColorEX2.glsl", "fsVtxColor.glsl");
	...
	m_uiTranLoc = glGetUniformLocation( m_uiProgram,  "vTranslate" );
...
void CQuadEX2B::SetAngle(float fAngle)
{
	m_fAngle = fAngle;
}
void CQuadEX2B::SetTranslate(float vT[])
{
	m_fTranslate[0] = vT[0];
	m_fTranslate[1] = vT[1];
	m_fTranslate[2] = vT[2];
...
void CQuadEX2B::Draw()
{
	...
	glUniform1f(m_uiAngleLoc, m_fAngle);
	glUniform3fv(m_uiTranLoc, 1, m_fTranslate);
